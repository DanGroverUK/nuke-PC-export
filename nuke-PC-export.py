# Written by Dan Grover, dan-grover@dan-grover.com
# If you have any comments, bugs, suggestions, questions, whatever - please email me!
...
def writeFile(fileOut):
    ...
    if nuke.selectedNode().Class() == 'BakedPointCloud':
        ...
        # We actually don't use the value below, but we need to get the first generator
        # value so that when we start getting the subsequent values, we have the useless
        # first value already out of the way.
        totalPointsRGB = int(next(rgbGen))
        ...
        csvFile.write(
            "float32 Position[0], float32 Position[1], float32 Position[2], float16 Color[0], float16 Color[1], float16 Color[2]\n")
        # Create a new task for the threader to run.
        task = nuke.ProgressTask("Saving Data...")
        # Loop through the number of points, defined by the first return from the generator functions
        for g in range(1, totalPoints):
            ...
            if task.isCancelled():
                ...
                return
            # Add a new line at the end of the loop to start on the next point
            lineToWrite = "{x},{y},{z},{r},{g},{b}\n".format(x=next(xyzGen), y=next(xyzGen), z=next(xyzGen),
                                                             r=next(rgbGen), g=next(rgbGen), b=next(rgbGen))
            ...
            if counter == 5000:
                ...
                print("Time taken to get to 5000 points: {}".format(processingTime))
            # if writeCounter == 300:
            #     writeString.close()
            #     writeString = cStringIO.StringIO()
            #     writeCounter = 0
        ...
        nuke.message("This isn't a BakedPointCloud node. If you have a PointCloudGenerator node, select some of the points, create a group within that node and then hit 'Bake Selected Groups'.")


def genUI():
    pcPanel = nuke.Panel("PC Export")
